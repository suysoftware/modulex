...
async def get_user_openai_tools(
    ...
    """Get user's authenticated tools in OpenAI tools format for Vercel AI SDK"""
...
# Move this to a different router or change the path structure
router_direct = APIRouter(prefix="/tools/direct", tags=["Direct Tool Execution"])

@router_direct.post("/{tool_name}/execute")
async def execute_tool_direct(
    tool_name: str = Path(..., description="Tool name with action (format: toolname_action)"),
    request: OpenAIToolExecutionRequest = None,
    user_id: str = Query(..., description="User ID"),
    db: AsyncSession = Depends(get_db)
):
    """Execute a tool directly (OpenAI/Vercel AI SDK format)"""
    tool_service = ToolService(db)
    
    try:
        # Get parameters from request body or use empty dict
        parameters = request.parameters if request and request.parameters else {}
        
        # Check if parameters include action and tool_key (from frontend)
        if "action" in parameters:
            action = parameters.pop("action")
            tool_key = parameters.pop("tool_key", None)
            
            if tool_key:
                # Use tool_key as the actual tool name
                actual_tool_name = tool_key
            else:
                # Parse tool name and action from the format: toolname_action
                if "_" not in tool_name:
                    raise ValueError(f"Invalid tool name format. Expected 'toolname_action', got '{tool_name}'")
